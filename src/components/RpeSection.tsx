...
import { buildRpeDetailedReport } from '../utils/rpeReport';
import { RpeDetailedReportView } from './RpeDetailedReportView';
...
// Same dot colours already used by the Wellness status legend in FitnessHubSection.
const WELLNESS_BAND_DOT_CLASS: Record<WellnessBand, string> = {
  green: 'bg-emerald-500',
  amber: 'bg-amber-500',
  red: 'bg-rose-500'
};

const WELLNESS_BAND_TITLE: Record<WellnessBand, string> = {
  green: 'Green',
  amber: 'Amber',
  red: 'Red'
};

const WellnessStatusDot: React.FC<{ band: WellnessBand | null }> = ({ band }) => {
  if (!band) return <span aria-label="Unknown">—</span>;
  return (
    <span
      role="img"
      aria-label={WELLNESS_BAND_TITLE[band]}
      title={WELLNESS_BAND_TITLE[band]}
      className={`inline-block h-2.5 w-2.5 shrink-0 rounded-full align-middle ${WELLNESS_BAND_DOT_CLASS[band]}`}
    />
  );
};
...
export const RpeSection: React.FC<RpeSectionProps> = ({ squadPlayers, trainingSessions }) => {
  ...
  const detailedReport = useMemo(
    () => (isReportOpen ? buildRpeDetailedReport({ entries, wellness: wellnessAnalysis }) : null),
    [isReportOpen, entries, wellnessAnalysis]
  );

  const handlePrintReport = () => {
    document.body.classList.add('print-rpe-report-mode');
    const cleanup = () => {
      document.body.classList.remove('print-rpe-report-mode');
  ...
  return (
    <div className="mt-5 space-y-5">
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2 self-start">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-xl bg-[#002142] px-3 py-2 text-xs font-black text-white shadow-sm hover:bg-[#00305f]"
          >
          ...
          <button
            type="button"
            onClick={() => setReloadToken((token) => token + 1)}
            className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700 shadow-sm hover:bg-slate-100"
          >
            <RefreshCw className="h-3.5 w-3.5" />
            Reload sheet
          </button>
      ...
      {isReportOpen && detailedReport ? (
        <RpeDetailedReportView
          report={detailedReport}
          wellnessBands={WELLNESS_BANDS}
          rpeBands={RPE_BANDS}
          bandLabels={BAND_LABELS}
          renderWellnessDot={(band) => <WellnessStatusDot band={band} />}
                  ...
                  <tr key={wellnessBand} className="border-t border-slate-100 font-semibold">
                    <td className="py-2 font-black text-slate-900">
                      <span className="inline-flex items-center gap-2">
                        <WellnessStatusDot band={wellnessBand} />
                        {BAND_LABELS[wellnessBand]}
                      </span>
                    </td>
                ...
                <tbody>
                  {wellnessAnalysis.points.slice(0, 40).map((point) => (
                    <tr key={`${point.playerId}-${point.dateKey}`} className="border-t border-slate-100 font-semibold">
                      ...
                      <td className="py-2">
                        <WellnessStatusDot band={point.wellnessBand} />
                      </td>
