...
type PhysioView = 'overview' | 'injuries' | 'new-injury' | 'edit-injury' | 'injury-detail' | 'complaints' | 'new-complaint' | 'history';
...
export function PhysiotherapySection({ userEmail }: PhysiotherapySectionProps) {
  ...
  if (loading || isLoadingAuthorization || loadingCreatePermission || loadingUpdatePermission || loadingDeletePermission) return <div className="grid min-h-[60vh] place-items-center bg-white"><div className="text-center"><LoaderCircle className="mx-auto h-7 w-7 animate-spin text-emerald-700" /><p className="mt-3 text-sm font-semibold text-slate-500">Loading clinical workspace…</p></div></div>;
  ...
  if (view === 'edit-injury' && selectedInjury) return <InjuryWorkflow teamId={selectedTeamId} players={context.players} sessions={context.sessions} matches={context.matches} previousInjuries={injuries.filter((injury) => injury.id !== selectedInjury.id)} injury={selectedInjury} onCancel={() => setView('injury-detail')} onSave={async (input) => { setSelectedInjury(await updateInjury(selectedInjury.id, input)); setView('injury-detail'); }} />;
  ...
  return <div className="min-h-[70vh] bg-[#f5f7f8]">
    <header className="border-b border-slate-800 bg-[#08233d] px-4 py-6 text-white sm:px-6">
      <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-start">
        ...
        {canCreate ? <button type="button" onClick={() => setView('new-injury')} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-rose-500 px-4 text-sm font-bold text-white hover:bg-rose-600"><Plus className="h-4 w-4" /> Record injury</button> : <div className="inline-flex items-center gap-2 rounded-lg bg-white/10 px-3 py-2 text-xs font-semibold text-slate-200"><Shield className="h-4 w-4" /> Read-only clinical access</div>}
      </div>
    ...
    <main className="px-4 py-7 sm:px-6">
      ...
      {!canCreate && !canWrite && !canDelete && <div className="mb-5 border-l-4 border-sky-500 bg-sky-50 p-4 text-sm text-sky-900">You can review clinical records. Creating or updating records requires confirmed clinical write access.</div>}
      {view === 'overview' && <PhysioOverview injuries={injuries} complaints={complaints} players={context.players} canWrite={canCreate} onNewInjury={() => setView('new-injury')} onOpenInjury={openInjury} onOpenInjuries={() => setView('injuries')} onOpenComplaints={() => setView('complaints')} />}
      {view === 'injuries' && <InjuryList injuries={injuries} players={context.players} onOpen={openInjury} />}
      {view === 'complaints' && <ComplaintList complaints={complaints} players={context.players} canWrite={canCreate} onNew={() => setView('new-complaint')} />}
