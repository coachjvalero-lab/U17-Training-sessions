            ...
            <form onSubmit={handleSaveMatch} className="space-y-3.5 text-xs font-medium text-slate-700">
              ...
              <div className="flex items-center justify-end space-x-2 pt-3 border-t border-slate-100">
                <button
                  ...
                  className="px-4 py-2 bg-slate-100 hover:bg-slate-200 disabled:bg-slate-50 disabled:text-slate-400 text-slate-700 font-bold rounded-xl text-xs"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={isSavingMatch}
                  className="px-5 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:bg-emerald-300 text-white font-extrabold rounded-xl text-xs shadow-md flex items-center space-x-2"
                >
                  {isSavingMatch && (
                    <div className="w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin" />
                  )}
                  <span>{isSavingMatch ? 'Saving...' : 'Save Match'}</span>
                </button>
              </div>
            </form>
          </div>
