...
async function ensureLegacyGkSessionsBackfilled(): Promise<void> {
  ...
  backfillPromise = (async () => {
    try {
      ...
      const { data: legacySessions, error: legacyErr } = await client
        .from('sessions')
      ...
      if (legacyErr) {
        console.warn('[GkSessionsService] Note querying legacy sessions:', legacyErr);
        return;
      }
      ...
      // Filter candidates that contain GK content or were legacy training sessions
      const candidates = (legacySessions as any[]).filter((s) =>
      ...
      // 2. Fetch existing GK sessions to ensure idempotency
      const { data: existingGk, error: gkErr } = await client
        .from(GK_SESSIONS_TABLE)
        .select('id,session_uid');

      if (gkErr) {
        console.warn('[GkSessionsService] Note querying existing gk_sessions:', gkErr);
        return;
      }
      ...
      const toMigrate = candidates.filter((s) => {
        const targetId = s.id.startsWith('gk-') ? s.id : `gk-${s.id}`;
        return !existingIds.has(targetId) && !existingIds.has(s.id);
      });
      ...
      const catalogRows = toMigrate.map((s) => ({
        session_uid: s.id,
        session_number: s.session_number || '',
        session_date: s.date || new Date().toISOString().slice(0, 10),
        source_legacy_session_id: s.id,
        created_at: s.updated_at || now,
        updated_at: s.updated_at || now
      }));

      const gkRows = toMigrate.map((s) => ({
        id: s.id.startsWith('gk-') ? s.id : `gk-${s.id}`,
        session_uid: s.id,
        legacy_session_id: s.id,
        team_name: s.team_name || DEFAULT_GK_TEAM_NAME,
        date: s.date || new Date().toISOString().slice(0, 10),
        time: s.time || '18:30 - 20:00',
        session_number: s.session_number || '',
        microcycle_day: s.microcycle_day || 'MD-3',
        main_objective: s.main_objective || '',
        materials_needed: s.materials_needed || '',
        observations: s.observations || null,
        squad_roster: s.squad_roster || [],
        attendance: s.attendance || [],
        warm_up: s.gk_warm_up || defaultBlock('warmup-block-gk', 'Warm Up'),
        main_part: s.gk_main_part || defaultBlock('main-block-gk', 'Main Part'),
        cool_down: s.gk_cool_down || defaultBlock('cooldown-block-gk', 'Cool Down'),
        player_groups: s.gk_player_groups || [],
        created_at: s.gk_updated_at || s.updated_at || now,
        updated_at: s.gk_updated_at || s.updated_at || now
      }));
      ...
      if (catErr) {
        console.warn('[GkSessionsService] Warning updating session_catalog for GK:', catErr);
      }
      ...
      if (insertErr) {
        console.warn('[GkSessionsService] Warning upserting gk_sessions:', insertErr);
      } else {
        console.log(`[GkSessionsService] Backfilled ${toMigrate.length} legacy GK session(s) into public.gk_sessions`);
      }
    } catch (err) {
      console.warn('[GkSessionsService] Error in ensureLegacyGkSessionsBackfilled:', err);
    }
  })();
...
export async function listGkSessions(): Promise<GkSession[]> {
  await ensureLegacyGkSessionsBackfilled();
